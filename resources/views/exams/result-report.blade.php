    ...
    <style>
        ...
        *, ::after, ::before {
        ...
        .marks {
            ...
            font-weight: {{ strpos($settings['marks_font_style'] ?? 'bold', 'bold') !== false ? 'bold' : 'bold' }};
        ...
        .teacher-name {
            ...
            font-weight: {{ strpos($settings['teacher_name_font_style'] ?? 'bold', 'bold') !== false ? 'bold' : 'bold' }};
        ...
        .subject-group {
            font-weight: {{ strpos($settings['subject_group_style'] ?? 'bold', 'bold') !== false ? 'bold' : 'bold' }};
...
<body>
    ...
    <div class="content">
        ...
        <div class="mt-3 row" style="position: relative;top: -100px;bottom: 5px">
    ...
    {{-- Subject results table starts --}}
            ...
            <div class="mt-3 row" style="position: relative;top: -110px">
    
            <div class=" col-10 ps-0">
            <table class="table table-bordered border-dark" style="margin-right: 400px">
                <thead class="text-center align-middle">
                    <tr style="height: 40px">
                        ...
                        <th rowspan="2"  style="">{{__("Remarks") .' '. __('and') .' '. __('signature') }}</th>
                    </tr>
                ...
                @foreach($examResultGroups as $group)
                    ...
                    @foreach($group->subjects as $subject)
                        ...
                        <tr class="text-center align-middle">
                            ...
                            @foreach($sequences as $sequence)
                                @if (isset($fetchSubjectData->sequence_marks->{$sequence->id}))
                                    @if ($fetchSubjectData->sequence_marks->{$sequence->id}>=10)
                                        <td class="marks">{{ number_format($fetchSubjectData->sequence_marks->{$sequence->id},2)}}</td>
                                    @else
                                        <td class="marks" style="color: #f00;">{{ number_format($fetchSubjectData->sequence_marks->{$sequence->id},2)}}</td>
                                    @endif
                                @else
                                    <td class="marks">/</td>
                                @endif
                            ...
                            @if ($fetchSubjectData->subject_avg >= 10)
                                <th class="marks">{{ $fetchSubjectData->subject_avg<0 ? "/" : number_format($fetchSubjectData->subject_avg,2) }}</th>
                            @else
                                <th class="marks"><span style="color: #f00">{{ $fetchSubjectData->subject_avg<0 ? "/" : number_format($fetchSubjectData->subject_avg,2) }}</span></th>
                            @endif
    
                            <!--Subject Weightage-->
    
                            <td class="marks">{{$subject->class_subject->weightage}}</td>
    
                            <!--N x C-->
                            <td class="marks">{{ $nXc<0 ? "/" : number_format($nXc,2) }}</td>
    
                            <!--Rank-->
                            <td class="marks">{{ $fetchSubjectData->subject_rank<0 ? "/" : $fetchSubjectData->subject_rank }}</td>
    
                            <!--Class Min marks-->
                            @if ($subject->class_details)
                                <td class="marks">{{ number_format($subject->class_details->min, 2) }}</td>
                            @else
                                <td class="marks">/</td>
                            @endif
                            ...
                            <!--Class Max marks-->
                            @if ($subject->class_details)
                                <td class="marks">{{ number_format($subject->class_details->max, 2) }}</td>
                            @else
                                <td class="marks">/</td>
                            @endif
                    ...
                    <tr class="text-center subject-group">
                        ...
                        @if($totalCoef!==0)
                            <td colspan="6" class="text-start">{{__("Moy / Av")}} : {{number_format($totalNxC / $totalCoef,2)}}</td>
                        @else
                            <td colspan="6" class="text-start">{{__("Moy / Av")}} : 0.00</td>
                        @endif
                    </tr>
                @endforeach
            </table>
    ...
    @if ($student->class_section->class->report_footer_table == "max")
        <div class="row col-12" style="position: relative;top: -113px;margin-bottom: 0;">
            <div class="col-3" style="padding-right: 10px;">
                <table class="table w-100" style="height: 195px">
                    <thead class="text-center align-middle border-1 border-dark">
                    <tr>
                        <th colspan="2">{{__("council_decision")}}</th>
                    </tr>
                    </thead>
                    <tbody class="border-1 border-dark">
                        <tr>
                            <td>{{__('total_absence')}}</td>
                            <td><span>{{$attendance != null ? $attendance->total_absences : '/'}}</span></td>
                        </tr>
                        <tr>
                            <td>{{__('justified_absence')}}</td>
                            <td><span>{{$attendance != null ? $attendance->justified_absences : '/' }}</span></td>
                        </tr>
                        <tr>
                            <td>{{__('unjustified_absence')}}</td>
                            <td><span>{{$attendance != null ? $attendance->unjustified_absences : '/' }}</span></td>
                        </tr>
                        <tr>
                            <td>{{ __('warning') }}</td>
                            <td>
            ...
            <div class="col-5" style="padding: 0 10px;">
                <table class=" table table-bordered border-dark w-100" style="height: 198px">
                    @php
            ...
            <div class="col-3" style="padding-left: 10px;">
                <table class="table table-bordered border-dark mb-0 w-50">
                    <thead class="text-center">
                    <th colspan="2">{{__("Performance Summary")}}</th>
                    </thead>
                    <tbody>
                    <tr>
                        <td>{{__("Avg")}}</td>
                        <td><b>{{number_format($studentClassPerformance->avg,2)}}</b></td>
                    </tr>
                    <tr>
                        <td>{{__("Rank")}}</td>
                        <td><b>{{ $studentClassPerformance->rank < 0 ? "NA" : $studentClassPerformance->rank . "  /  ". number_format($classPerformance->class_size,0)}}</b></td>
                    </tr>
                    <tr>
                        <td>{{__("Honor")}}</td>
                        <td>
                ...
                <table class="table-bordered border-dark w-50" style="margin-top: 10px">
                    <thead class="text-center">
                    ...
                    <tbody>
                        <tr>
                            <td>{{__("Class Avg")}}</td>
                            <td><b>{{number_format($classPerformance->class_avg,2)}}</b></td>
                        </tr>
                    <tr>
                        <td>{{__("Min Avg")}}</td>
                        <td><b>{{number_format($classPerformance->min_avg,2)}}</b></td>
                    </tr>
                    <tr>
                        <td>{{__("Max Avg")}}</td>
                        <td><b>{{number_format($classPerformance->max_avg,2)}}</b></td>
                    </tr>
    ...
    @else
        ...
        <div class="row g-0" style="position: relative;top: -125px;">
            {{-- Council Decision --}}
    ...
    {{-- Subject results table ends --}}
     ...
     {{--Signatures table  --}}
    <div class="col-12 row" style="position: relative;top: -117px;">
    ...
    <div style="font-style: italic; margin-top: -120px; font-size: 12px">
        <hr>    
        <div>
            <b>
