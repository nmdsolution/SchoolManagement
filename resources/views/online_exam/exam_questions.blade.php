...
@section('content')
    <div class="content-wrapper">
        <div class="page-header">
            ...
            <a class="btn btn-sm btn-primary" href="{{ route('online-exam.index') }}">{{ __('back') }}</a>
        </div>
        <div class="row grid-margin">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        <form class="pt-3 mt-6" id="add-new-question-online-exam" method="POST" action="{{ route('online-exam.add-new-question') }}" novalidate>
                            <div class="row">
                                <input type="hidden" name="class_subject_id" value="{{ $online_exam_db->class_subject_id }}">
                                <div class="form-group col-md-4">
                                    <label>{{ __('class') }}</label>
                                    <input type="text" id="add-question-class" value="{{ $online_exam_db->class_subject->class->name }}" placeholder="{{ __('class') }}" class="form-control" readonly/>
                                </div>
                                <div class="form-group col-md-4">
                            ...
                            <div class="add-new-question-container" style="display:none">
                                <div class="bg-light p-4">
                                    <div class="form-group">
                                        <button type="button" class="btn btn-danger mb-2 d-flex float-right remove-add-new-question">
                                ...
                                <input class="btn btn-primary mt-4" id="new-question-add" type="submit" value={{ __('add') }}>
                            </div>
                        </form>
                        <div class="">
                            <button type="button" class="btn btn-primary ml-3 add-new-question-button">{{ __('add_new_question') }}</button>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form class="pt-3 mt-6" id="store-assign-questions-form" method="POST" action="{{ route('online-exam.store-choice-question') }}">
                            ...
                            <div id='questions_block' class="form-group mt-4" style="overflow-y:scroll;height:700px;overflow-x: hidden;">
                                <ol id="sortable-row">
                                        ...
                                        @foreach ($exam_questions as $questions)
                                            @if ($questions->questions->question_type)
                                                <div class="list-group row">
                                                    ...
                                                    <li id="q{{ $questions->question_id }}" class="list-group-item justify-content-between align-items-center ui-state-default list-group-item-secondary m-2">
                                                        {{ $questions->question_id }}
                                                        <hr class="question-divider">
                                                        <div>
                                                            <textarea class="equation-editor-inline" name="qc{{ $questions->question_id }}">{{ htmlspecialchars_decode($questions->questions->question) }} What numbers are there while multiplying to reach result 4? What numbers are there while multiplying to reach result 4?</textarea>
                                                        </div>
                                                        <span class="text-right row">
                                                            <div class="col-md-10">
                                                                <input type="number" placeholder="Marks" class="list-group-item" name="assign_questions[{{ $questions->question_id }}][marks]" style="width: 20%;" value="{{ $questions->marks }}">
                                                            </div>
                                            ...
                                            @else
                                                <div class="list-group row">
                                                    ...
                                                    <li id="q{{ $questions->question_id }}" class="list-group-item justify-content-between align-items-center ui-state-default list-group-item-secondary m-2">
                                                        {{ $questions->question_id }}
                                                        <hr class="question-divider">
                                                        <span class="text-center">{{ htmlspecialchars_decode($questions->questions->question) }}</span>
                                                        <span class="text-right row">
                                                            <div class="col-md-10">
                                                                <input type="number" placeholder="Marks" class="list-group-item col-md-6" name="assign_questions[{{ $questions->question_id }}][marks]" style="width: 20%;" value="{{ $questions->marks }}">
                                                            </div>
                                                            <div class="col-md-2">
                                                                <a class="btn btn-danger btn-sm remove-row ml-2" data-edit_id="{{ $questions->id }}" data-id="{{ $questions->question_id }}">
                                                                <i class="fa fa-times" aria-hidden="true"></i>
                                ...
                                <input class="btn btn-primary ml-4 submit_questions_btn" type="submit" value={{ __('submit') }} />
                            </div>
                        </form>
                    </div>
                </div>
