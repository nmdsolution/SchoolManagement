                ...
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="form-group col-12 local-forms">
                                ...
                                <select required name="class_section_id" id="timetable_class_section"
                                        class="col-md-6 col-sm-12 form-control select" style="width:100%;" tabindex="-1"
                                        aria-hidden="true">
                        ...
                        <div id="timetable-div" class="d-none">
                            <ul class="nav nav-tabs timetable_nav " role="tablist">
                                <li class="nav-item">
                                    <a class="nav-link day_menu active" id="monday-tab" data-id="monday" data-bs-toggle="tab"
                                       href="#monday" role="tab" aria-controls="monday"
                                       aria-selected="false">Monday</a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link day_menu" id="tuesday-tab" data-id="tuesday" data-bs-toggle="tab"
                                       href="#tuesday" role="tab" aria-controls="tuesday"
                                       aria-selected="false">Tuesday</a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link day_menu" id="wednesday-tab" data-id="wednesday" data-bs-toggle="tab"
                                       href="#wednesday" role="tab" aria-controls="wednesday"
                                       aria-selected="false">Wednesday</a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link day_menu" id="thursday-tab" data-id="thursday" data-bs-toggle="tab"
                                       href="#thursday" role="tab" aria-controls="thursday"
                                       aria-selected="false">Thursday</a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link day_menu" id="friday-tab" data-id="friday" data-bs-toggle="tab" href="#friday"
                                       role="tab" aria-controls="friday" aria-selected="false">Friday</a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link day_menu" id="saturday-tab" data-id="saturday" data-bs-toggle="tab"
                                       href="#saturday" role="tab" aria-controls="saturday"
                                       aria-selected="false">Saturday</a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link day_menu" id="sunday-tab" data-id="sunday" data-bs-toggle="tab" href="#sunday"
                                       role="tab" aria-controls="sunday" aria-selected="false">Sunday</a>
                                </li>
                            </ul>

                            <div class="tab-content">
                                <div class="tab-pane fade active show" id="monday" role="tabpanel"
...
@section('js')
    <script>
                ...
                success: function (response) {
                    ...
                    if (response != '') {
                        ...
                        for (let i = 0; i < response.length; i++) {
                            html += '<option value=' + response[i]['subject']['id'] + '>' + response[i][
                                'subject'
                                ]['name'] + ' - ' + response[i]['subject']['type'] + '</option>';
                        }
                    } else {
        ...
        window.onload = $('.day_name').trigger('click');

        
        $('.day_menu').click(function (e) { 
            e.preventDefault();
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response !== '') {
                        set_subject();
                        for (let i = 0; i < response.length; i++) {
                            // REMOVE input-group FOR RESPONSIVE LAYOUT
                            // <div class="input-group col-sm-12 col-md-2">
                            var responseNote = response[i]['note'] ?? "";

                            html += '<div data-repeater-item="" class="row mb-2 ' + day_name + '_count" id="' +
                                day_name + '_count[' + i + ']">' +
                                '<input required hidden type="text" value="' + response[i]['id'] + '" name="' +
                                day_name + '_group[' + i + '][id]" id="' + day_name + '_id[' + i +
                                ']" class="form-control" placeholder="Id">' +
                                '<div class="col-sm-12 col-md-2">' +
                                '<select required name="' + day_name + '_group[' + i +
                                '][subject_id]" id="subject" class="form-control set_subject_id ' + day_name +
                                '_group[' + i +
                                '][subject_id]" style="width:100%;" tabindex="-1" aria-hidden="true"></select>' +
                                '</div>' +
                                '<div class="col-sm-12 col-md-2">' +
                                '<select required name="' + day_name + '_group[' + i +
                                '][teacher_id]" id="teacher" class="form-control set_teacher_id" style="width:100%;" tabindex="-1" aria-hidden="true"></select>' +
                                '</div>' +
                                '<div class="col-sm-12 col-md-2">' +
                                '<input required type="time" value="' + response[i]['start_time'] + '" name="' +
                                day_name + '_group[' + i +
                                '][start_time]" class="timetable_start_time form-control" placeholder="Start time">' +
                                '</div>' +
                                '<div class="col-sm-12 col-md-2">' +
                                '<input required type="time" value="' + response[i]['end_time'] + '" name="' +
                                day_name + '_group[' + i +
                                '][end_time]" class="timetable_end_time form-control" placeholder="End time">' +
                                '</div>' +
                                '<div class="col-sm-12 col-md-2">' +
                                '<input type="text" value="' + responseNote + '" name="' + day_name +
                                '_group[' + i + '][note]" class="form-control" placeholder="Note">' +
                                '</div>' +
                                '<div class="col-sm-12 col-md-2"><button data-repeater-delete type="button" class="mt-2 row_remove btn btn-gradient-danger btn-sm icon-btn ml-2"><i class="fa fa-trash"></i></button></div>' +
                                '</div>'
                        }
                        ...
                        for (let j = 0; j < response.length; j++) {
                            setTimeout(function () {
                                $('select[name^="' + day_name + '_group[' + j + '][subject_id]"]')
                                    .children('option[value="' + response[j]['subject_teacher'][
                                        'subject'
                                        ]['id'] + '"]').attr("selected", true);
                                $('select[name^="' + day_name + '_group[' + j + '][subject_id]"]')
                                    .trigger('change');
                                setTimeout(function () {
                                    $('select[name^="' + day_name + '_group[' + j +
                                        '][teacher_id]"]').children('option[value="' +
                                        response[j]['subject_teacher']['teacher']['id'] +
                                        '"]').attr("selected", true);
                                }, 1000)
                            }, 1000)
                        }
