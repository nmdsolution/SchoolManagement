...
class StudentApiController extends Controller {
    public function login(Request $request) {
        ...
        if (Auth::attempt(['email' => $request->gr_number, 'password' => $request->password])) {
            ...
            $user = $auth->load(['student.class_section','student.class_section.class.stream']);
            ...
            $user->class_section_name = $user->student->class_section->full_name;
            //Set Medium name
    ...
    public function getParentDetails(Request $request) {
        try {
            ...
            $guardian = Guardian::where('student_id', $student->id)->pluck('user_id');
            $parent_guardian = Parents::whereIn('user_id', $guardian)->get();

            $data = array(
                'father'   => (!empty($student->father)) ? $student->father : (object)[],
                'mother'   => (!empty($student->mother)) ? $student->mother : (object)[],
                'guardian' => (!empty($parent_guardian)) ? $parent_guardian : (object)[]
            );
            $response = array(
                'error'   => false,
    ...
    public function getExamList(Request $request) {
        try {
            ...
            $exam_data_db = ExamClassSection::with('exam.session_year:id,name')->where('class_section_id', $student->class_section->id)
    ...
    public function getExamDetails(Request $request) {
        ...
        try {
            ...
            $exam_data_db = Exam::with([
                'timetable' => function ($q) use ($request, $class_section_id) {
                    $q->where(['exam_id' => $request->exam_id, 'class_section_id' => $class_section_id])->with(['subject'])->orderby('date');
                }
        ...
        try {
            ...
            if (sizeof($exam_result_db)) {
                foreach ($exam_result_db as $exam_result_data) {
                    $starting_date_db = ExamTimetable::select(DB::raw("min(date)"))->where(['exam_id' => $exam_result_data->exam_id, 'class_section_id' => $class_data->class_section->id])->first();
                    $starting_date = $starting_date_db['min(date)'];
                    ...
                    $exam_result = array(
                        ...
                        'class_name'     => $class_data->class_section->full_name,
        ...
        try {
            ...
            if ($check_start_date != 0) {
                $response = array(
                    ...
                    'code'    => trans('exam_not_started_yet'),
                );
                return response()->json($response);
            }
    ...
    public function getOnlineExamReport(Request $request) {
        ...
        try {
            ...
            if (isset($exam_exists) && !empty($exam_exists)) {
                ...
                $online_exams_db = OnlineExam::with([
    ...
    //     } 
    //     $input=$request->all();
    //     $rowexists=CourseStudent::where('course_id',$request->course_id)->where('student_id',$request->student_id)->first();
    //     if(!$rowexists)
    //     {
...
//            $session_year_id = $session_year['session_year'];
//
//            $fees_paid = FeesPaid::where(['student_id' => $student->id, 'session_year_id' => $session_year_id])->with('student.user:id,first_name,last_name', 'class');
//            if ($fees_paid->count()) {
//                $response = array(
...
//Get Fees Details
    ...
    {
        try {
            $student_data = $request->user()->student->load('class_section');
            ...
            foreach ($fees_class as $data) {
                if ($data->choiceable == 1) {
                    $paid_optional_data = FeesChoiceable::owner()->where(['student_id' => $student_data->id, 'class_id' => $class_id, 'session_year_id' => $session_year_id, 'fees_type_id' => $data['fees_type_id']])->first();
                    $optional_fees_data[] = array(
            ...
            if ($installment_db->count()) {
                ...
                foreach ($installment_db_data as $data) {
                    $paid_installment_data = PaidInstallmentFee::owner()->where(['student_id' => $request->child_id, 'class_id' => $class_id, 'session_year_id' => $session_year_id, 'installment_fee_id' => $data['id']])->first();
                    $installment_data[] = array(
                        'id' => $data->id,
                        'name' => $data->name,
            ...
            $session_year_data = SessionYear::owner()->where('id', $session_year_id)->first();
    ...
    public function feesPaidList(Request $request): \Illuminate\Http\JsonResponse
    {
        try {
            $student = $request->user()->student;
            $fees_paid = FeesPaid::query()->where(['student_id' => $student->id])->with('session_year:id,name', 'class.medium')->get();
    ...
    public function getFeesPaymentTransactions(Request $request): \Illuminate\Http\JsonResponse
    {
        try {
            $student = $request->user()->student;
            $fees_payment_trasactions = PaymentTransaction::where('student_id', $student->id)->with('session_year')->with(['student' => function ($q) {
