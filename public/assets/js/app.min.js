...
(function($) {
	...
	function init() {
		...
		$(document).ready(function(){
			setTimeout(function(){
				$('#sidebar-menu ul li.submenu a.active').each(function(index,element){
					$(element).parent('li').parent('ul').siblings('a').trigger('click');
				})
			},500);
		})
	}
	...
	if($('.toggle-password-1').length > 0) {
		$(document).on('click', '.toggle-password-1', function() {
			$(this).toggleClass("feather-eye feather-eye-off");
			var input = $(".pass-input-1");
			if (input.attr("type") == "password") {
				input.attr("type", "text");
			} else {
				input.attr("type", "password");
